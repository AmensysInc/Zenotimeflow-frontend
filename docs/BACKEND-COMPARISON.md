...
The frontend calls `http://localhost:8000/api` (see `src/lib/api-client.ts`). Use **zeno-time-backend** as the single backend.
...
3. Open the frontend at http://localhost:8080 and sign in. If you still see errors, check the **browser Network tab**: the response may show a traceback. If the **Python path** in the traceback mentions `zeno_time_flow_backend`, the wrong backend is still running on 8000 — stop it and run only from `zeno-time-backend`.

**Clock In (employee login):** To see the React Native Clock In flow at http://localhost:8080/clock, run Expo as well: `cd mobile && npx expo start --web --port 8081`. See `docs/DEPLOY-WEB-WITH-CLOCK.md` for details.
